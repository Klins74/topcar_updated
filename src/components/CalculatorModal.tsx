...
type DurationOption = {
  label: string;
  value: string; // формат 'price_id-duration'
};
...
export default function CalculatorModal({ isOpen, onClose }: CalculatorModalProps) {
  ...
  useEffect(() => {
    if (isOpen) { // Загружаем данные только когда модальное окно открыто
  ...
  // Логика расчета цены и доступных опций
  const { availableDurations, totalPrice, selectedPriceInfo } = useMemo(() => {
    ...
    const durationOptions: DurationOption[] = relevantPrices.map(p => {
        ...
        return {
            label: durationLabel,
            value: `${p.id}-${p.days_from}`, // Уникальный ключ для выбора
        }
    });
  ...
  return (
    <>
      ...
      {/* Модальное окно бронирования, которое откроется после калькулятора */}
      {bookingInfo && (
        <BookingModal 
          isOpen={!!bookingInfo}
