// Файл: src/components/CarModal.tsx (Финальная версия с публичными промокодами)
'use client'
...
export default function CarModal({ isOpen, onClose, car }: CarModalProps) {
  ...
  const handlePrimaryBookAction = () => {
    if (!selectedTariff) alert('Пожалуйста, выберите тариф');
  ...
  return (
    <>
          ...
          <div className="grid md:grid-cols-2 flex-grow min-h-0">
            ...
            <div className="flex flex-col p-6 sm:p-8 overflow-y-auto min-h-0 scrollbar-thin scrollbar-thumb-neutral-700">
              ...
              <div className="space-y-5 flex-grow">
                ...
                {car.pricing.transfer && <PriceOption label="Трансфер" value={car.pricing.transfer} isSelected={selectedTariff?.serviceType === 'Трансфер'} onClick={() => handleTariffSelect({ serviceType: 'Трансфер', duration: 'Поездка', price: car.pricing.transfer! })} />}
              </div>
      ...
      {showBookingModal && selectedTariff && (
        <BookingModal isOpen={true} onClose={() => setShowBookingModal(false)} carName={car.name} bookingDetails={{ ...selectedTariff, price: finalPrice }} promoCode={appliedPromo?.code} />
      )}
    </>
  )
}