...
// import Link from 'next/link'; // REMOVED: Link was not used in this file's JSX
...
export default function AdminCarsPage() {
  ...
  if (!adminUser) { // This check happens after useEffect has run at least once
    return (
  ...
  const FormInputField = ({id, label, type = "text", placeholder, value, onChange, required = true, accept, min}: FormInputFieldProps) => (
    <div>
        ...
        <input
            ...
            accept={accept}
            min={min}
            className="w-full py-2.5 px-3 text-sm text-white bg-neutral-800 border border-neutral-600 rounded-lg 
              ...
              >
                ...
                {formMessage && (
                    <div className={`flex items-center gap-2 p-3 rounded-md text-sm mt-1
                        ${formMessage.type === 'success' ? 'bg-green-900/30 border border-green-700/50 text-green-300' : ''}
                        ${formMessage.type === 'error' ? 'bg-red-900/30 border border-red-700/50 text-red-300' : ''}
                    `}>
                ...
                <button
                  ...
                  className="w-full sm:w-auto mt-2 px-8 py-3 bg-[#d4af37] text-black rounded-lg text-sm sm:text-base font-semibold 
                  ...
                  {cars.map((car) => (
                    <li
                      ...
                      className="bg-neutral-800/60 border border-neutral-700/70 rounded-xl shadow-lg 
                                 overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-neutral-600"
                    >
